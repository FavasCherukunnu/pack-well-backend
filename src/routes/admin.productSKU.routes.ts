...
const productSKUcreateSchema = z.object({
    Sku: z.object({
        ...
        m06_price: z.union([z.number(), z.string()]).refine(
            (val) => !isNaN(Number(val)) && Number(val) >= 1,
            { message: "must be a number greater than or equal to 1" }),
        m06_m04_product_category_id: z.union([z.number(), z.string()]).refine(
            (val) => !isNaN(Number(val)) && Number(val) >= 1,
            { message: "must be a number greater than or equal to 1" }),
        m06_quantity: z.union([z.number(), z.string()]).refine(
            (val) => !isNaN(Number(val)),
            { message: "must be a number" }),
        m06_is_new: z.union([z.number(), z.string()]).refine((val) => [0, 1, '0', '1'].includes(val), { message: "must be 0 or 1" }),
        m06_single_order_limit: z.union([z.number(), z.string()]).optional().refine(
            (val) => {
                console.log(val)
                if (val) {
...
router.post('/product-sku',
    ...
    validateDataMiddleWare(productSKUcreateSchema),
    createSKUController
);
...
router.put('/product-sku/:id',
    verifyJWTorRefreshJWT,
    uploadProductImages.fields([{ name: 'm06_thumbnail_image', maxCount: 1 }]),
    validateDataMiddleWare(productSKUUpdateSchema),
    updateSKUController
);


router.get('/product-sku/:id',
    verifyJWTorRefreshJWT,
    readSKUController
);
...
router.put('/product-sku-image-reorder/:skuId',
    verifyJWTorRefreshJWT,
    validateDataMiddleWare(productSKUReorderImageSchema),
    reorderProductSkuImagesController
);
router.delete('/product-sku-image/:id',
    verifyJWTorRefreshJWT,
    deleteProductSkuImageController
);
...
router.post('/product-sku-image',
    verifyJWTorRefreshJWT,
    upload.any(),
    validateDataMiddleWare(productSKUCreateImageSchema),
    createProductSkuImageController
);
