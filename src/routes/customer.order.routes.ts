...
router.post('/order/request-order-otp',
    validateDataMiddleWare(requestOtpSchema),
    requestOrderOtpController
);
router.post('/order/verify-and-confirm-order',
    validateDataMiddleWare(verifyAndConfirmOrderSchema),
    verifyAndConfirmOrderController
);
router.get('/get-user-orders',
    verifyJWTorRefreshJWTUser,
