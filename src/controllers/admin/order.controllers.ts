...
export const listOrdersAdminController = async (req: Request<{}, {}, {}, { page?: number, limit?: number, searchTerm?: string }>, res: Response, next: NextFunction) => {
    try {
...
export const readOrderAdminController = async (req: TypedRequestBody<{ orderId: string }>, res: Response, next: NextFunction) => {
    try {
...
export const updateOrderStatusByOrderIdAdminController = async (req: Request<{ orderId: string }, {}, { statusId: number }>, res: Response, next: NextFunction) => {
    try {
        const { orderId } = req.params;
        ...
        await OrderStatusHistoryModel.updateMany(
            { O05_O03_order_id: order._id, O05_O02_status: { $in: statusesToDelete }, deleted_at: null },
