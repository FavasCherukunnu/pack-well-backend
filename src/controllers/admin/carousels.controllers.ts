...
export const carouselListAdminController = async (req: Request, res: Response, next: NextFunction) => {
    try {
...
export const createCarouselAdminController = async (req: Request<{}, {}, { P02_name: string }>, res: Response, next: NextFunction) => {
    try {
        const { P02_name, } = req.body;
        const carouselImage = ((req.files as any)?.P02_image?.[0] as Express.Multer.File) || null;
        ...
        if (carouselImage && !['image/png', 'image/jpeg', 'image/jpg'].includes(carouselImage.mimetype)) {
            ...
            return res.status(400).json(
                new ApiResponse(false, 400, "Product image type must be png, jpg or jpeg", null)
            );
        }
        ...
        if(!cloudinaryResponse?.url){
            return res.status(400).json(
                new ApiResponse(false, 400, "Error uploading image to cloudinary", null)
            )
        }
...
export const updateCarouselAdminController = async (req: Request, res: Response, next: NextFunction) => {
    try {
        const { id } = req.params;
        ...
        if (!existingCarousel) {
            return res.status(404).json(
                new ApiResponse(false, 404, "Carousels not found", null));
        }
        ...
        const newCarousel = await CarouselModel.findByIdAndUpdate(
            id,
            {
                P02_name,
...
export const deleteCarouselAdminController = async (
    ...
    try {
        ...
        const deleted = await CarouselModel.findByIdAndUpdate(
            id,
            {
                deleted_at: new Date(),
            },
...
export const reorderCarouselsAdminController = async (req: Request<{},{},{
    carousels: {
        _id: string;
    }[]
    ...
    try {
        ...
        if (!Array.isArray(carousels) || carousels.length === 0) {
            return res.status(400).json(
                new ApiResponse(false, 400, "Carousels not found in the request", null)
            );
        }
        ...
        if (carouselsFound.length !== carousels.length) {
            return res.status(400).json(
                new ApiResponse(false, 400, "The carousels you are trying to reorder do not exist", null)
            );
        }
        ...
        const updatedCarousels = await CarouselModel.bulkWrite(
            carouselsToUpdate.map(carousel => ({
                updateOne: {
