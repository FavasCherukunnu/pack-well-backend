...
export const ListContactUsController = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    return res.status(200).json(
      new ApiResponse(true, 200, "Contact us fetched successfully", {
        enquiries: contactUs,
        pagination:{
...
export const ReadContactusController = async (req: Request, res: Response, next: NextFunction) => {
  try {
    const { id } = req.params;
...
export const ContactusMarkAsReadController = async (req: Request, res: Response, next: NextFunction) => {
  try {
    const { id } = req.params;
