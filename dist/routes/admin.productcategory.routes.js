...
import { uploadProductCategory } from "../middlewares/multer.middleware.js";
const router = Router();
...
router.post('/product-category', verifyJWTorRefreshJWT, uploadProductCategory.fields([{ name: "m04_image", maxCount: 1 }]), validateDataMiddleWare(productCategorySchema), createProductCategoryController);
router.delete('/product-category/:id', verifyJWTorRefreshJWT, deleteProductCategoryController);
router.put('/product-category/:id', verifyJWTorRefreshJWT, uploadProductCategory.fields([{ name: "m04_image", maxCount: 1 }]), validateDataMiddleWare(productCategorySchema), updateProductCategoryController);
router.get('/product-category', verifyJWTorRefreshJWT, validateParamsMiddleWare(z.object({
...
router.get('/product-category/:id', verifyJWTorRefreshJWT, readProductCategoryController);
