...
export const createProductVariationController = async (req, res, next) => {
    try {
...
export const updateProductVariationController = async (req, res, next) => {
    try {
...
export const deleteProductVariationController = async (req, res, next) => {
    try {
        const id = req.params.id;
...
export const createProductVariationOptionController = async (req, res, next) => {
    try {
...
export const updateProductVariationOptionController = async (req, res, next) => {
    try {
        const { M09_name, M09_is_active, M09_M08_product_variation_id } = req.body;
        const id = req.params.id;
        ...
        variationOption.M09_M08_product_variation_id = new ObjectId(M09_M08_product_variation_id);
...
export const deleteProductVariationOptionController = async (req, res, next) => {
    try {
        const id = req.params.id;
...
export const updateVariationConfigurationController = async (req, res, next) => {
    ...
    try {
        ...
        const existingConfigsMap = new Map(existingConfigs.map(config => [config._id.toString(), config]));
        ...
        const variationMap = new Map(variations.map(variation => [variation._id.toString(), variation]));
        ...
        const pipeline = [
            {
                $match: {
                    M10_M05_product_id: new ObjectId(existingConfigs[0].M10_M05_product_id.toString()),
