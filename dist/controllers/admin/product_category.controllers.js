...
export const createProductCategoryController = async (req, res, next) => {
    ...
    try {
        ...
        const image_path = req.files?.m04_image?.[0]?.path || null;
        ...
        res.status(201).json(new ApiResponse(true, 201, "Product category created successfully", productCategory));
    }
    catch (error) {
        console.log(error);
        if (image_path)
...
export const deleteProductCategoryController = async (req, res, next) => {
    try {
        const { id } = req.params;
...
export const listProductCategoryController = async (req, res, next) => {
    try {
...
export const readProductCategoryController = async (req, res, next) => {
    try {
        const { id } = req.params;
...
export const updateProductCategoryController = async (req, res, next) => {
    ...
    try {
        ...
        const cloudinary_image = req.files?.m04_image?.[0]?.path || null;
        const productCategory = await prisma.m04_product_category.findUnique({
            where: { id: Number(id), deleted_at: null }
        });
        if (!productCategory) {
