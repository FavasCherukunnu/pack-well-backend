...
export const getVariationByProductIdCustomerController = async (req, res, next) => {
    try {
        ...
        return res.status(200).json(new ApiResponse(true, 200, "Variation fetched successfully", variations));
    }
    catch (error) {
        next(error);
    }
};
export const getProductSKUsWithVariationsController = async (req, res, next) => {
    try {
        const productId = new ObjectId(req.params.productId);
        ...
        return res.status(200).json(new ApiResponse(true, 200, "SKUs with variations fetched successfully", skus));
    }
    catch (error) {
        next(error);
    }
